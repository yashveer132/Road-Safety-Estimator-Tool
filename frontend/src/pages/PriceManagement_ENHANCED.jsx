...
export default function PriceManagement(){
  ...
  const search = () => {
    setLoading(true)
    axios.get((import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api/prices/search', { 
      params: { query } 
    })
      .then(r => {
        ...
        const avgPrice = data.length > 0 
          ? data.reduce((sum, item) => sum + (item.unitPrice || 0), 0) / data.length 
          : 0
    ...
    <Box className="fade-in">
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" sx={{ color: 'text.secondary' }}>
          Search and manage material prices from CPWD SOR, GeM Portal, and other sources
        </Typography>
      </Box>
      ...
      >
        ...
        {loading ? (
          ...
          >
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>
              {query ? 'Try adjusting your search terms' : 'Enter keywords to search for material prices'}
            </Typography>
          </Box>
        ) : (
          <>
